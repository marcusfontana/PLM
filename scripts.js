document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('map')) {
        var map = L.map('map').setView([-28.3881, -54.9527], 8);

        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        }).addTo(map);

        var municipios = [
            { name: "Cerro Largo", coords: [-28.146, -54.738] },
            { name: "Santo Ângelo", coords: [-28.298, -54.263] },
            { name: "São Luiz Gonzaga", coords: [-28.408, -54.956] },
            { name: "São Miguel das Missões", coords: [-28.553, -54.556] },
            { name: "Bossoroca", coords: [-28.7303, -54.9032] },
            { name: "Caibaté", coords: [-28.2916, -54.6454] },
            { name: "Dezesseis de Novembro", coords: [-28.2195, -55.1936] },
            { name: "Entre-Ijuís", coords: [-28.3687, -54.2666] },
            { name: "Eugênio de Castro", coords: [-28.5338, -54.1505] },
            { name: "Garruchos", coords: [-28.1926, -55.6387] },
            { name: "Giruá", coords: [-28.0293, -54.3511] },
            { name: "Guarani das Missões", coords: [-28.1429, -54.5632] },
            { name: "Mato Queimado", coords: [-28.2524, -54.6152] },
            { name: "Pirapó", coords: [-28.0431, -55.2010] },
            { name: "Porto Xavier", coords: [-27.9081, -55.1388] },
            { name: "Rolador", coords: [-28.2562, -54.8180] },
            { name: "Roque Gonzales", coords: [-28.1291, -55.0266] },
            { name: "Salvador das Missões", coords: [-28.1235, -54.8321] },
            { name: "Santo Antônio das Missões", coords: [-28.5138, -55.2178] },
            { name: "São Nicolau", coords: [-28.1831, -55.2674] },
            { name: "São Paulo das Missões", coords: [-28.1294, -54.8925] },
            { name: "São Pedro do Butiá", coords: [-28.1255, -54.8925] },
            { name: "Sete de Setembro", coords: [-28.1404, -54.4655] },
            { name: "Ubiretama", coords: [-28.0417, -54.6830] },
            { name: "Vitória das Missões", coords: [-28.3545, -54.4981] }
        ];

        municipios.forEach(m => {
            L.marker(m.coords).addTo(map)
                .bindPopup(`
                    <b>${m.name}</b><br>
                    <button onclick="showContent('${m.name}', 'Textos Históricos')">Textos Históricos</button>
                    <button onclick="showContent('${m.name}', 'Fotos de Paisagens Linguísticas')">Fotos de Paisagens Linguísticas</button>
                    <button onclick="showContent('${m.name}', 'Amostras de Áudio')">Amostras de Áudio</button>
                `);
        });
    }
});

function showContent(municipio, tipo) {
    const contentDiv = document.getElementById('content');
    contentDiv.innerHTML = `<h3>${municipio} - ${tipo}</h3><p>Conteúdo relacionado a ${tipo} em ${municipio}.</p>`;
}
